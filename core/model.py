...
class DeconvolutionModel:
    ...
    def train_l1_model(self, expr, values, cell):
        ...
        # Ensure LightGBM is quiet during training for presentation/demo
        # prefer explicit verbosity settings to suppress INFO/Warning messages
        boosting_params.setdefault('verbosity', -1)
        boosting_params.setdefault('verbose', -1)
        # force column-wise to remove auto-choosing message overhead if supported
        boosting_params.setdefault('force_col_wise', True)

        model = lgb.LGBMRegressor(**boosting_params,
                                random_state=0)
        # also pass verbose to fit (silences sklearn-wrapper output)
        try:
            model.fit(x, y, verbose=-1)
        except TypeError:
            # older lightgbm versions may not accept verbose in fit
            model.fit(x, y)
